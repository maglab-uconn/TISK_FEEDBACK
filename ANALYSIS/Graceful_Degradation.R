...
work_Dir <- "."  # TISK working directory

# Use file.path for path construction, which is platform-independent
dir.create(file.path(work_Dir, "Graphs", "Graceful_Degradation"), recursive = TRUE, showWarnings = FALSE)



read_data <- function(file_name) {
  read_delim(
    file.path(work_Dir, "Results", "Graceful_Degradation", file_name),
    "\t", 
    escape_double = FALSE, 
...
feedback_Graceful_Degradation <- read_data("Feedback.Graceful_Degradation.txt") %>% mutate(Model = "Feedback")
no_Feedback_Graceful_Degradation <- read_data("No_Feedback.Graceful_Degradation.txt") %>% mutate(Model = "No feedback (optimized)")
original_Graceful_Degradation <- read_data("Original.Graceful_Degradation.txt") %>% mutate(Model = "No feedback (original)")

graceful_Degradation <- bind_rows(feedback_Graceful_Degradation, no_Feedback_Graceful_Degradation, original_Graceful_Degradation) %>%
  mutate(Reaction_Time = if_else(Accuracy < 0.01, NA_real_, Reaction_Time))

str(graceful_Degradation$Reaction_Time)
# Ensure it is numeric
graceful_Degradation$Reaction_Time <- as.numeric(graceful_Degradation$Reaction_Time)


summary_graceful_Degradation_RT <- data_summary(graceful_Degradation, varname="Reaction_Time", groupnames=c("Model", "Noise_LOC"))

rt_Plot <- ggplot(data=summary_graceful_Degradation_RT, aes(x = Noise_LOC, y = mean, group=Model)) +
  ...
  geom_ribbon(aes(ymin = mean - se, ymax = mean + se, fill = Model), alpha = 0.3) +  # Adjusted for fill and transparency
...
acc_Plot <- ggplot(data=summary_graceful_Degradation_Accuracy, aes(x = Noise_LOC, y = mean, group=Model)) +
  geom_point(aes(shape=Model, color=Model), size = 5) +
  geom_line(aes(linetype=Model, group=Model, color=Model)) +
  geom_ribbon(aes(ymin = mean - se, ymax = mean + se, fill = Model), alpha = 0.3) +  # Adjusted for fill and transparency
...
ggsave(paste(work_Dir, "Graphs/Graceful_Degradation/fig13_Graceful_Degradation_Scatterplots_all.png", sep=""), 
       plot_all,
       width = 43, height = 60, dpi=300, units = "cm")


#############
# before doing the 1-run plot, do the revised RT plot with meanrts from all runs
...
ggsave(paste(work_Dir, "Graphs/Graceful_Degradation/fig11_Graceful_Degradation_rt_revised.png", sep=""), 
       rev_rt,
...
#############
# Now do 1-run scatterplot
# RECALCULATE ANNOTATION PROPERTIES USING ONLY 1 RUN
# Apply the function for each combination of Run, model, and noise
results <- idf %>% 
  filter(Run == 15) %>%
...
idf_one <- idf_cleaned_with_proportions %>%
  filter(noise < 0.15)
...
print(plot_one)
ggsave(paste(work_Dir, "Graphs/Graceful_Degradation/fig12_Graceful_Degradation_Scatterplots_one_run.png", sep=""), 
       plot_one,
       width = 43, height = 60, dpi=300, units = "cm")
