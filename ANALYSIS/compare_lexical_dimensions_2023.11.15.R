...
trace.tab<-read.table("TRACEDATA/trace_slex.txt",header=TRUE,na.strings="NA")
#trace.tab<-read.table("TRACEDATA/trace_accrt.txt.lexstat",header=TRUE,na.strings="NA")
...
head(combined_df)
model_compare <- combined_df %>%
  dplyr::select(Target, traceRT, tiskfbRT, tisknofbRT, tiskorRT)
...
textsize=20
tiskfb_tisk_plot <-
  ggplot(model_compare,aes(x=tiskfbRT,y=tiskorRT)) + geom_point(alpha=.5,cex=3,position="jitter",color="grey1") +
  geom_abline(intercept=0,slope=1,color="grey") +
  coord_cartesian(ylim=c(starting, ending), xlim=c(starting,ending)) +
  ...
  # title=element_text(size=textsize))
  #plot.title = element_text(size=8) +
  xlab("RT (TISKfb cycles)") + ylab("RT (TISK [no fb] cycles)") +
  #stat_smooth(method="lm", se=FALSE) +
  annotate("text", x=20, y=80, label="r = 0.97",size=8)
tiskfb_tisk_plot

tisknofb_tiskor_plot <-
  ggplot(model_compare,aes(x=tisknofbRT,y=tiskorRT)) + geom_point(alpha=.5,cex=3,position="jitter",color="grey1") +
  geom_abline(intercept=0,slope=1,color="grey") +
  coord_cartesian(ylim=c(starting, ending), xlim=c(starting,ending)) +
  geom_smooth(method='lm', se=TRUE, linetype="dashed") +
  theme(panel.background = element_rect(colour="black", fill="white"),
  ...
  # title=element_text(size=textsize))
  #plot.title = element_text(size=8) +
  xlab("RT (TISK NO fb cycles)") + ylab("RT (TISK cycles)") +
  #stat_smooth(method="lm", se=FALSE) +
  annotate("text", x=20, y=80, label="r = 0.97",size=8)
tisknofb_tiskor_plot

tiskfb_trace_plot <-
  ggplot(model_compare,aes(x=tiskfbRT,y=traceRT)) + geom_point(alpha=.5,cex=3,position="jitter",color="grey1") +
  geom_abline(intercept=0,slope=1,color="grey") +
  coord_cartesian(ylim=c(starting, ending), xlim=c(starting,ending)) +
  geom_smooth(method='lm', se=TRUE, linetype="dashed") +
  theme(panel.background = element_rect(colour="black", fill="white"),
...
tiskor_trace_plot <-
  ggplot(model_compare,aes(x=tiskorRT,y=traceRT)) + geom_point(alpha=.5,cex=3,position="jitter",color="grey1") +
  geom_abline(intercept=0,slope=1,color="grey") +
  coord_cartesian(ylim=c(starting, ending), xlim=c(starting,ending)) +
  geom_smooth(method='lm', se=TRUE, linetype="dashed") +
  theme(panel.background = element_rect(colour="black", fill="white"),
  ...
  # title=element_text(size=textsize))
  #plot.title = element_text(size=8) +
  xlab("RT (TISK [no fb] cycles)") + ylab("RT (TRACE cycles)") +
  #stat_smooth(method="lm", se=FALSE) +
  annotate("text", x=20, y=80, label="r = 0.84",size=8)
tiskor_trace_plot
...
#pdf("compare_model_rts_rev.pdf",width=15,height=5)
