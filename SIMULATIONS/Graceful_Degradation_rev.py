...
def Graceful_Degradation(model, pronunciation_List, participants = 16, noise_LOC_Step = 0.005, noise_LOC_Step_Count = 15, prefix="",modelname="mnERR"):    
    result_List_Dict = {};
    ...
    for index in range(participants):
        print("{}Participant_{}....".format(prefix, index), file=sys.stderr)
        _, _, _, _, reaction_Time, accuracy = model.Run_List(pronunciation_List, batch_Size = 212, wordstring=f'{index},{modelname}',dump=True)
        ...
        for noise_LOC in np.arange(noise_LOC_Step, noise_LOC_Step * (noise_LOC_Step_Count + 1), noise_LOC_Step):
            _, _, _, _, reaction_Time, accuracy = model.Run_List(pronunciation_List, noise = (noise_LOC, noise_LOC / 2), wordstring=f'{index},{modelname}',dump=True);
