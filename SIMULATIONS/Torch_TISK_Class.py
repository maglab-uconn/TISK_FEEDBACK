...
class TISK_Model:
    def __init__(self, phoneme_List, word_List, time_Slots = None, nPhone_Threshold = None, use_gpu= False):
        #Assign Label
        ...
        if use_gpu:
            if torch.cuda.is_available():
                ...
                logging.warning('GPU is not available. Model is on the CPU mode.')
    ...
    def Weight_Initialize(self):
        ...
        #Weight Generate
        weightMatrix_Phoneme_to_Diphone = np.zeros(shape=(self.phoneme_Amount * self.parameter_Dict["time_Slots"], self.diphone_Amount))
        weightMatrix_Phoneme_to_Single_Phone = np.zeros(shape=(self.phoneme_Amount * self.parameter_Dict["time_Slots"], self.phoneme_Amount))
        weightMatrix_Diphone_to_Word = np.zeros(shape=(self.diphone_Amount, self.word_Amount))
        weightMatrix_Single_Phone_to_Word = np.zeros(shape=(self.phoneme_Amount, self.word_Amount))
        weightMatrix_Word_to_Word = np.zeros(shape=(self.word_Amount, self.word_Amount))
        weightMatrix_Word_to_Diphone = np.zeros(shape=(self.word_Amount, self.diphone_Amount))
        weightMatrix_Word_to_Single_Phone = np.zeros(shape=(self.word_Amount, self.phoneme_Amount))
            ...
            for phoneme_Index in range(self.phoneme_Amount):
                for diphone_Index in range(self.diphone_Amount):
                    if self.phoneme_List[phoneme_Index] == self.diphone_List[diphone_Index][0]:
                        weightMatrix_Phoneme_to_Diphone[slot_Index * self.phoneme_Amount + phoneme_Index, diphone_Index] += self.parameter_Dict[("Weight", "Phoneme_to_Phone")] * (self.parameter_Dict["time_Slots"] - 1 - slot_Index)    #When slot is more later, weight decrease more.
                    if self.phoneme_List[phoneme_Index] == self.diphone_List[diphone_Index][1]:
                        weightMatrix_Phoneme_to_Diphone[slot_Index * self.phoneme_Amount + phoneme_Index, diphone_Index] += self.parameter_Dict[("Weight", "Phoneme_to_Phone")] * slot_Index #When slot is more later, weight increase more.
                for single_Phone_Index in range(self.phoneme_Amount):
                    if self.phoneme_List[phoneme_Index] == self.single_Phone_List[single_Phone_Index]:
                        weightMatrix_Phoneme_to_Single_Phone[slot_Index * self.phoneme_Amount + phoneme_Index, single_Phone_Index] += self.parameter_Dict[("Weight", "Phoneme_to_Phone")] * self.parameter_Dict["time_Slots"]    #Always weight become 1
        ...
        for diphone_Index in range(self.diphone_Amount):
            for word_Index in range(self.word_Amount):
                if self.diphone_List[diphone_Index] in self.Open_Diphone_Generate(self.word_List[word_Index]):
                    weightMatrix_Diphone_to_Word[diphone_Index, word_Index] = self.parameter_Dict[("Weight", "Diphone_to_Word")] / len(self.word_List[word_Index])   #Divide by the length of pronunciation
        ...
        for single_Phone_Index in range(self.phoneme_Amount):
            for word_Index in range(self.word_Amount):
                if self.single_Phone_List[single_Phone_Index] in self.word_List[word_Index]:
                    weightMatrix_Single_Phone_to_Word[single_Phone_Index, word_Index] = self.parameter_Dict[("Weight", "SPhone_to_Word")] #Always weight become 0.01
        ...
        if self.parameter_Dict[("Weight", "Word_to_Word")] != 0:
            for word1_Index in range(self.word_Amount):
                for word2_Index in range(self.word_Amount):
                    ...
                    weightMatrix_Word_to_Word[word1_Index, word2_Index] = len(intersection) # shared feature is more, the inhibition also become stronger
            for word_Index in range(self.word_Amount):
                weightMatrix_Word_to_Word[word_Index, word_Index] = 0 # self inhibtion is 0
            weightMatrix_Word_to_Word *= self.parameter_Dict[("Weight", "Word_to_Word")]
        ...
        if self.parameter_Dict[("Feedback", "Word_to_Diphone_Activation")] != 0 or self.parameter_Dict[("Feedback", "Word_to_Diphone_Inhibition")] != 0 or self.parameter_Dict[("Feedback", "Word_to_SPhone_Activation")] != 0 or self.parameter_Dict[("Feedback", "Word_to_SPhone_Inhibition")] != 0:
            for word_Index in range(self.word_Amount):
                for diphone_Index in range(self.diphone_Amount):
                    if self.diphone_List[diphone_Index] in self.Open_Diphone_Generate(self.word_List[word_Index]):
                        weightMatrix_Word_to_Diphone[word_Index, diphone_Index] = self.parameter_Dict[("Feedback", "Word_to_Diphone_Activation")]
                    else:
                        weightMatrix_Word_to_Diphone[word_Index, diphone_Index] = self.parameter_Dict[("Feedback", "Word_to_Diphone_Inhibition")]
                for single_Phone_Index in range(self.phoneme_Amount):
                    if self.single_Phone_List[single_Phone_Index] in self.word_List[word_Index]:
                        weightMatrix_Word_to_Single_Phone[word_Index, single_Phone_Index] = self.parameter_Dict[("Feedback", "Word_to_SPhone_Activation")]
                    else:
                        weightMatrix_Word_to_Single_Phone[word_Index, single_Phone_Index] = self.parameter_Dict[("Feedback", "Word_to_SPhone_Inhibition")]

        self.weightMatrix_Phoneme_to_Diphone = torch.FloatTensor(weightMatrix_Phoneme_to_Diphone).to(self.device)
        self.weightMatrix_Phoneme_to_Single_Phone = torch.FloatTensor(weightMatrix_Phoneme_to_Single_Phone).to(self.device)
        self.weightMatrix_Diphone_to_Word = torch.FloatTensor(weightMatrix_Diphone_to_Word).to(self.device)
        self.weightMatrix_Single_Phone_to_Word = torch.FloatTensor(weightMatrix_Single_Phone_to_Word).to(self.device)
        self.weightMatrix_Word_to_Word = torch.FloatTensor(weightMatrix_Word_to_Word).to(self.device)
        self.weightMatrix_Word_to_Diphone = torch.FloatTensor(weightMatrix_Word_to_Diphone).to(self.device)
        self.weightMatrix_Word_to_Single_Phone = torch.FloatTensor(weightMatrix_Word_to_Single_Phone).to(self.device)
    ...
    def Run(self, pronunciation, activation_Ratio_Dict = {}):
        ...
        using_Pattern = self.Pattern_Generate(pronunciation, activation_Ratio_Dict).to(self.device)
        ...
        ##Gate initialize
        gate_Phoneme_to_Diphone = torch.ones(
            size= (self.phoneme_Amount*self.parameter_Dict["time_Slots"], self.diphone_Amount)
            ).to(self.device)   #Initially all gates have state 1

        ##Layer Initialize
        phoneme_Layer_Activation = torch.zeros(
            size= (1, self.phoneme_Amount * self.parameter_Dict["time_Slots"])
            ).to(self.device)
        diphone_Layer_Activation = torch.zeros(
            size= (1, self.diphone_Amount)
            ).to(self.device)
        single_Phone_Layer_Activation = torch.zeros(
            size= (1, self.phoneme_Amount)
            ).to(self.device)
        word_Layer_Activation = torch.zeros(
            size= (1, self.word_Amount)
            ).to(self.device)

        for slot_Index in range(self.parameter_Dict["time_Slots"]):
            location_Input = torch.zeros(
                size= (1, self.phoneme_Amount * self.parameter_Dict["time_Slots"])
                ).to(self.device)
            ...
            for step_Index in range(self.parameter_Dict["iStep"]):
                ...
                diphone_Layer_Stroage = phoneme_Layer_Activation @ (gate_Phoneme_to_Diphone * self.weightMatrix_Phoneme_to_Diphone)
                diphone_Layer_Stroage = \
                    torch.sign((torch.sign(diphone_Layer_Stroage - self.parameter_Dict["nPhone_Threshold"]) + 1) / 2) / 10 + \
                    word_Layer_Activation @ self.weightMatrix_Word_to_Diphone #Binary + Feedback
                single_Phone_Layer_Stroage = phoneme_Layer_Activation @ self.weightMatrix_Phoneme_to_Single_Phone
                single_Phone_Layer_Stroage = \
                    torch.sign((torch.sign(single_Phone_Layer_Stroage - self.parameter_Dict["nPhone_Threshold"]) + 1) /2) / 10 + \
                    word_Layer_Activation @ self.weightMatrix_Word_to_Single_Phone  #Binary + Feedback
                word_Layer_Stroage = \
                    diphone_Layer_Activation @ self.weightMatrix_Diphone_to_Word + \
                    single_Phone_Layer_Activation @ self.weightMatrix_Single_Phone_to_Word + \
                    word_Layer_Activation @ self.weightMatrix_Word_to_Word

                phoneme_Layer_Activation = torch.clamp(
                    phoneme_Layer_Activation * (1 - self.parameter_Dict[("Decay", "Phoneme")]) - \
                    torch.abs(phoneme_Layer_Stroage) * phoneme_Layer_Activation + \
                    phoneme_Layer_Stroage.clip(min=0),
                    min= 0,
                    max= 1
                    )
                diphone_Layer_Activation = torch.clamp(
                    diphone_Layer_Activation * (1 - self.parameter_Dict[("Decay", "Diphone")]) - \
                    torch.abs(diphone_Layer_Stroage) * diphone_Layer_Activation + \
                    diphone_Layer_Stroage.clip(min=0),
                    min= 0,
                    max= 1
                    )
                single_Phone_Layer_Activation = torch.clamp(
                    single_Phone_Layer_Activation * (1 - self.parameter_Dict[("Decay", "SPhone")]) - \
                    torch.abs(single_Phone_Layer_Stroage) * single_Phone_Layer_Activation + \
                    single_Phone_Layer_Stroage.clip(min=0),
                    min= 0,
                    max= 1
                    )
                word_Layer_Activation = torch.clamp(
                    word_Layer_Activation * (1 - self.parameter_Dict[("Decay", "Word")]) - \
                    torch.abs(word_Layer_Stroage) * word_Layer_Activation + \
                    word_Layer_Stroage.clip(min=0),
                    min= 0,
                    max= 1
                    )
        ...
        return \
            torch.cat(phoneme_Activation_Cycle_List, dim= 0).cpu().numpy(), \
            torch.cat(diphone_Activation_Cycle_List, dim= 0).cpu().numpy(), \
            torch.cat(single_Phone_Activation_Cycle_List, dim= 0).cpu().numpy(), \
            torch.cat(word_Activation_Cycle_List, dim= 0).cpu().numpy()
    ...
    def Multi_Run(self, pronunciation_List):
        ...
        using_Pattern = torch.cat(pattern_List, dim= 0).to(self.device)
        ...
        ##Gate initialize
        gate_Phoneme_to_Diphone = torch.ones(
            size= (len(pronunciation_List), self.phoneme_Amount * self.parameter_Dict["time_Slots"], self.diphone_Amount)
            ).to(self.device)   #Initially all gates have state 1

        ##Layer Initialize
        phoneme_Layer_Activation = torch.zeros(
            size= (len(pronunciation_List), self.phoneme_Amount * self.parameter_Dict["time_Slots"])
            ).to(self.device)
        diphone_Layer_Activation = torch.zeros(
            size= (len(pronunciation_List), self.diphone_Amount)
            ).to(self.device)
        single_Phone_Layer_Activation = torch.zeros(
            size= (len(pronunciation_List), self.phoneme_Amount)
            ).to(self.device)
        word_Layer_Activation = torch.zeros(
            size= (len(pronunciation_List), self.word_Amount)
            ).to(self.device)
        ...
        for slot_Index in range(self.parameter_Dict["time_Slots"]):
            ...
            for step_Index in range(self.parameter_Dict["iStep"]):
                ...
                diphone_Layer_Stroage = (phoneme_Layer_Activation[:, None, :] @ gated_WeightMatrix_Phoneme_to_Diphone)[:, 0, :]   #Because weight is 3D.
                diphone_Layer_Stroage = \
                    torch.sign((torch.sign(diphone_Layer_Stroage - self.parameter_Dict["nPhone_Threshold"]) + 1) /2) / 10 + \
                    word_Layer_Activation @ self.weightMatrix_Word_to_Diphone   #Binary + Feedback
                single_Phone_Layer_Stroage = phoneme_Layer_Activation @ self.weightMatrix_Phoneme_to_Single_Phone
        ...
        phoneme_Activation_Cycle = torch.stack(phoneme_Activation_Cycle_List, dim= 1).cpu().numpy()
        diphone_Activation_Cycle = torch.stack(diphone_Activation_Cycle_List, dim= 1).cpu().numpy()
        single_Phone_Activation_Cycle = torch.stack(single_Phone_Activation_Cycle_List, dim= 1).cpu().numpy()
        word_Activation_Cycle = torch.stack(word_Activation_Cycle_List, dim= 1).cpu().numpy()
    ...
    def RT_Time_Dependent(self, pronunciation, word_Activation_Array, time_criterion = 10, diff_criterion=0.05):
        target_Index = self.word_List.index(pronunciation)
        target_Array = word_Activation_Array[:,target_Index]
        other_Max_Array = np.max(np.delete(word_Activation_Array, (target_Index), 1), axis=1)
        check_Array = target_Array >= other_Max_Array + diff_criterion

        for cycle in range(self.parameter_Dict["time_Slots"] * self.parameter_Dict["iStep"] - time_criterion):
            if all(check_Array[cycle:cycle + time_criterion]):
                # Check if the target word's activation is greater than all other words from this cycle to the end
                if all(target_Array[cycle:] > np.max(np.delete(word_Activation_Array[cycle:], (target_Index), 1), axis=1)):
                    return cycle #+ time_criterion
...
#                return cycle + time_criterion
    ...
    def old_RT_Time_Dependent(self, pronunciation, word_Activation_Array, criterion = 10):
        target_Index = self.word_List.index(pronunciation)
        target_Array = word_Activation_Array[:,target_Index]
        other_Max_Array = np.max(np.delete(word_Activation_Array, (target_Index), 1), axis=1)
    ...
    def Run_List(self, pronunciation_List, absolute_Acc_Criteria=0.75, relative_Acc_Criteria=0.05, time_Acc_Criteria=10, output_File_Name=None, raw_Data=False, categorize=False, reaction_Time=False, batch_Size=100, noise=(0, 0)):
        ...
        for batch_Index in range(0, len(pronunciation_List), batch_Size):
            ...
            phoneme_Activation_Array, diphone_Activation_Array, single_Phone_Activation_Array, word_Activation_Array = self.Multi_Run_Noise(pronunciation_List[batch_Index:batch_Index + batch_Size], noise)
        ...
        if not output_File_Name is None:            
            os.makedirs(os.path.join(os.getcwd(), os.path.split(output_File_Name)[0]).replace("\\", "/"), exist_ok=True)
    ...
    def Category_List(self, pronunciation):
        ...
        for word in self.word_List:
            ...
            # updating to include deletion rhymes -- e.g., kat and at
            if ( (pronunciation[1:] == word[1:] and pronunciation[0] != word[0]) or
                 (pronunciation[1:] == word) or (pronunciation == word[1:]) ) :
            ...
            if pronunciation != word and not word in cohort_List and not word in rhyme_List:
    ...
    def Average_Activation_by_Category_Graph(self, pronunciation_List, file_Save = False, output_File_Name = "Average_Activation_by_Category_Graph.png", batch_Size=100):
        ...
        plt.title("Average flow by category")
        plt.gca().set_xlim([0, self.parameter_Dict["time_Slots"] * self.parameter_Dict["iStep"]])
        plt.gca().set_ylim([-0.01,1.01])
        plt.legend()
        plt.draw()
    ...
    #The functions to test the progressive noise
    def Multi_Run_Noise(self, pronunciation_List, noise):
        pattern_List = []
        for pronunciation in pronunciation_List:
            pattern_List.append(self.Pattern_Generate(pronunciation))
        using_Pattern = torch.cat(pattern_List, dim= 0).to(self.device)
        ...
        for slot_Index in range(self.parameter_Dict["time_Slots"]):
            ...
            for step_Index in range(self.parameter_Dict["iStep"]):
                ...
                diphone_Layer_Activation = torch.clamp( # this part is for noise
                    diphone_Layer_Activation + \
                    torch.empty_like(diphone_Layer_Activation).normal_(mean= noise[0], std= noise[1]),
                    min= 0,
                    max= 1
                    )
    ...
    def Run_List_Noise(self, pronunciation_List, absolute_Acc_Criteria=0.75, relative_Acc_Criteria=0.05, time_Acc_Criteria=10, batch_Size=212, noise=(0, 0)):
        spent_Time_List = []
...
if __name__ == "__main__":
    ...
    tisk_Model.Display_Graph("^^^^^^^^l", display_Diphone_List = ["^^", "^l", "l^", "ll"], file_Save=True)
